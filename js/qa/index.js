    ...
    <div class="header">
      <div class="inner">
        ...
        <div class="nav">
          ...
          <a href="" v-bind:title="user.name"><img v-bind:src="user.avatar" class="avatar"/></a>
        </div>
      </div>
    </div>
  `
    ...
    <div class="question-card">
      ...
      <div class="answers">
        ...
        <div class="answer-author">
          <a>
            <img v-bind:src="question.answer.author.avatar">{{question.answer.author.name}}
          </a>
          &nbsp;|&nbsp;
  ...
  template: `
    <div class="hot-question-item">
      ...
      <div class="answers">
        <div class="answer-author">
          <a href="">
            <img v-bind:src="question.answer.author.avatar">
            <span class="name">{{question.answer.author.name}}</span>
            &nbsp;|&nbsp;
            <span class="desc">{{question.answer.author.desc}}</span>
          </a>
        </div>
        <div v-if="question.answer.pictures.length > 0" class="answer-pic">
          <img v-bind:src="question.answer.pictures[0]">
...
Vue.component("user-card", {
  ...
  template: `
    <div class="card user-card">
      ...
      <div>
        <ul>
          <li><a href="">我的回答{{user.num_answer}}</a></li>
          <li><a href="">我的收藏{{user.num_collect}}</a></li>
          <li><a href="">我的邀约{{user.num_invite}}</a></li>
          <li><a href="">我的关注{{user.num_follow}}</a></li>
          <li><a href="">我的粉丝{{user.num_fans}}</a></li>
        </ul>
      </div>
    </div>
  `
...
Vue.component("creator-card", {
  ...
  template: `
    <div class="card creator-card">
      <a v-bind:href="'./user.html?uid=' + user.id">
        <div class="creator-entrance-text">
          创作者中心
        </div>
        <div class="analytics">
          <div>
            <div>昨日回答阅读数</div>
            <div>{{user.num_read}}</div>
          </div>
          <div>
            <div>昨日回答赞同数</div>
            <div>{{user.num_agree}}</div>
          </div>
        </div>
      </a>
    </div>
...
user = {
      ...
      name: "季冬", 
      desc: "东南大学计算机专业研究生",
      avatar: "http://p9-tt.byteimg.com/large/pgc-image/3e675927d59a4257967100ac34df2a3f",
      num_answer: 10,
      num_collect: 100,
      num_follow: 45,
      num_fans: 300,
